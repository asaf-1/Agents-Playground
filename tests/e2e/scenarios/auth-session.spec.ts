...
test.describe("Auth session", () => {
  ...
  test("signs in from the login page and lands on the dashboard", async ({ loginPage, page }) => {
  ...
  test("rejects wrong credentials with an inline error", async ({ loginPage }) => {
    await loginPage.goto();
    await loginPage.login("alice@demo.local", "wrong-password");
    await expect(loginPage.error).toHaveText("Invalid email or password.");
  });

  test("authenticated admin sees the session banner on a protected page", async ({ page }) => {
    await page.goto("/profile");
    await expect(page.getByTestId("session-banner")).toHaveText("Signed in as Alice Northwind (Admin)");
    await expect(page).toHaveURL(/\/profile$/);
  });

  test("an expired session redirects a protected page to login (auth-or-session)", async ({ page, context, baseURL }) => {
