...
class FakeSelfHealingProvider implements SelfHealingLlmProvider {
  ...
  constructor(
    private readonly decideWith: (input: SelfHealingLlmProviderInput) => SelfHealingLlmDecision
  ) {}
...
function joinNowDecision(input: SelfHealingLlmProviderInput): SelfHealingLlmDecision {
  const candidate = input.allowedCandidates.find((item) => item.text.includes("Join Now"));

  return {
    allowedAction: "click",
    confidence: 0.96,
    rationale: "The Join Now CTA best matches the dashboard/start intent and is present in the bounded candidate list.",
    recommendation: "act",
...
function hasUsableOpenAiKey() {
  ...
  return (
    Boolean(apiKey) &&
    !/your-openai-api-key|sk-your-key|<key>|placeholder|replace-me/i.test(apiKey)
  );
}
...
test("real self-healing LLM agent recovers a stale CTA against the live app", async ({
  ...
  try {
    await page.locator('button:has-text("Start Free Trial")').click({ timeout: 400 });
    initialFailure = "The stale Start Free Trial selector unexpectedly resolved.";
  } catch (error) {
  ...
  const agent = new SelfHealingLlmAgent(page, provider, { mode: "bounded-action" });
  ...
  expect(provider.calls[0].allowedCandidates.some((candidate) => candidate.text.includes("Join Now"))).toBeTruthy();
...
test("Obsidian memory agent writes a real healing run log into the vault", async () => {
  ...
  const writeResult = await agent.writeHealingRunLog({
    ...
    scenario: "real-agent-proof",
    validation: {
      command: "npx.cmd playwright test tests/e2e/scenarios/real-agent-proof.spec.ts",
      passed: true,
...
test("Obsidian memory agent writes a workspace state log for session handoff", async () => {
  ...
  const writeResult = await agent.writeWorkspaceStateLog({
    changedFiles: [
      ...
      {
        description: "Public project entry point updated when the real-agent feature was added.",
        path: "README.md",
        status: "modified"
      }
    ],
...
test("Obsidian closeout agent writes a workspace report when required docs are updated", async () => {
  ...
  const result = await agent.closeout({
    summary: "Closeout agent proof captured code, test, README, and vault documentation changes.",
...
test("disabled self-healing LLM mode makes no provider call", async ({ page }) => {
  const provider = new FakeSelfHealingProvider(joinNowDecision);
...
test("@live-openai live OpenAI provider returns a bounded structured self-healing decision", async ({
  ...
  const result = await agent.recover({
    action: "click",
    failureEvidence: {
      errorMessage: "The stale Start Free Trial selector failed against the current landing page.",
      staleSelector: 'button:has-text("Start Free Trial")',
      targetType: "button"
    },
    intentTokens: ["join", "dashboard", "start"],
  ...
  const healingLog = await obsidian.writeHealingRunLog({
    ...
    scenario: "real-agent-proof-live-openai",
    validation: {
      command: 'npx.cmd playwright test tests/e2e/scenarios/real-agent-proof.spec.ts --grep "@live-openai"',
      passed: true,
      summary: "Live OpenAI provider returned a bounded structured decision in report-only mode."
    }
  });
  const workspaceLog = await obsidian.writeWorkspaceStateLog({
    changedFiles: [
      {
        description: "Live OpenAI smoke generated real Obsidian evidence for this run.",
    ...
    summary: "Live OpenAI real-agent smoke passed and wrote Obsidian vault evidence.",
