...
- **Self-updating flow catalog**: `flow-catalog.yml` regenerates
  `scripts/test-runner/flow-catalog.json` and the Actions `flow` dropdown on
  every push to `main`, committing only when the flow set changed. Currently
  67 flows / 157 E2E tests across three tiers (groups, spec files, describe
  blocks).
...
## Blocking a real first run

Nothing here is a code problem. These are the steps that make it real:

1. **Push the work.** The workflow does not exist on GitHub yet, so no run can
   be dispatched. Per repo policy this goes on a feature branch, through a PR,
   with `PR Validation / Pre-Merge Gate` green.
2. **Create a fine-grained PAT** scoped to this one repository, Actions: read
   and write, nothing else. That is the runner's only credential.
3. **Set the app's env** (`TR_GITHUB_TOKEN`, `TR_REPO`, `TR_INVITE_CODE`, and
   `TR_SECURE_COOKIE=true` behind HTTPS). See `test-runner/.env.example`.
4. **Deploy the app somewhere always-on** so colleagues can reach it while the
   owner's machine is off. `test-runner/Dockerfile` is ready; mount a volume at
