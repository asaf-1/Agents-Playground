...
- The **six Playwright/Claude agents** under `.claude/agents/` — Markdown agent definitions driven by a harness through the `playwright-test` MCP server. Section 6.
...
## 6. The six Playwright / Claude agents
...
| ------------- | ------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| senior leader | `.claude/agents/playwright-test-senior-leader.md` | custom   | Flattens a goal into AI-native pods, specialist handoff briefs, validation gates, and closeout criteria  |
...
Pipeline: `senior leader -> pod plan -> planner/generator or diagnostician -> (healer | reporter)`. Drift heals; by-design defects (the intentional RBAC over-permission and broken-product demo defects) get **reported**, never papered over. Full definitions and the workspace-agnostic adoption variant live in `.claude/agents/` and `md/PORTABLE_AGENT_ADOPTION_GUIDE.md`.

## 7. Codex and skill support

The Claude custom-agent surface and Codex skill surface are both supported:

| Surface             | Path                                              | Use                                                                                 |
| ------------------- | ------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Claude custom agent | `.claude/agents/playwright-test-senior-leader.md` | Use from a Claude-compatible harness as the senior leader agent                     |
| Codex skill         | `.agents/skills/senior-leader/SKILL.md`           | Use from Codex as the senior leader pod-orchestration workflow                      |
| Claude skill mirror | `.claude/skills/senior-leader/SKILL.md`           | Use where Claude slash/skill workflows are preferred over the custom-agent registry |

The skill files do not replace `.claude/agents/`; they give Codex and Claude a shared operator workflow for the same senior-leader pattern.
