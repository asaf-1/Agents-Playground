...
- **Playwright 1.61.1 + Node-pin lift (2026-06-27):** upgraded `@playwright/test` and `playwright` to `1.61.1` (the standalone `playwright`, imported by `BugReportingAgent`, now lives in `devDependencies`). Host CI moved 20 → 24. The QA Docker runner image (`Dockerfile.e2e`) is intentionally left at `v1.59.1-noble` because Docker stays disabled, so `main-validation`'s containerized full-regression remains red until that image is bumped — host PR validation, the canary, and the local suite are all green.
