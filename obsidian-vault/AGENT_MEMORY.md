...
- **k8s canary's first real run failed (2026-09-22):** a manual dispatch of `.github/workflows/k8s-canary.yml` (run 35764386609) failed at "Install kind". The step downloads with `curl -o ./kind`, but the checkout already has a `kind/` folder (`kind/kind-config.yaml`), so curl exits with code 23. The fix, pending as Phase 1 of the Terraform plan in `STATUS.md`, is to download to `$RUNNER_TEMP`. Every earlier post-merge run passed only because `kubernetes.enabled: false` makes the job skip after about 7s.
