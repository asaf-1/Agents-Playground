...
const productState = new URLSearchParams(window.location.search).get("state") || "valid";

const productLayout = document.querySelector("[data-testid='product-layout']");
const productStateElement = document.querySelector("[data-testid='product-state']");
...
const productSummary = document.querySelector("[data-testid='product-summary']");
...
async function loadProduct() {
  ...
  const response = await fetch(`/api/product/${productId}?state=${productState}`);
  const payload = await response.json();
  const product = payload.product;
  const numericPrice = Number(product.price);

  productLayout.classList.toggle("product-layout--broken", Boolean(product.layout?.overlap));
  productStateElement.textContent = payload.state === "broken" ? "Broken state" : "Valid state";
  productStateElement.classList.toggle("status-pill--warning", payload.state === "broken");
  productStateElement.classList.toggle("status-pill--success", payload.state !== "broken");
